...
* 데이터 조작어(DML, date manipulation language) - select, insert, update, delete
...
* 스키마 표현 
	<모델링의 1차 작업>
	M - 수강생(고유번호, 이름, 성별, 나이, 전화번호, 과정이름, 과정반, 주소, 전공, 수업목적)
...
* 사용자 계정 만드는 쿼리
> create user 'conang'@'%' identified by 'test123'; // conang 이라는 이름의 사용자 계정을 만들고 @ '%' : ip에서 접근 가능
> grant all privileges on memberdb.* to 'conang'@'%'; // nang 계정에 mamberdb에만 접근 가능하도록 권한을 주겠다.
